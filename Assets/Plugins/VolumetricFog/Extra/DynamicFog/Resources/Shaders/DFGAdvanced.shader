...
Shader "DynamicFog/Advanced" {
	...
	fixed4 computeSkyColor(fixed4 color, float3 worldPos) {
		...
		fixed4 skyFogColor = lerp(_FogColor, _FogColor2, saturate(worldPos.y / _FogHeightData.x));
		return step(worldPos.y,-0.01) * skyFogColor + // below ground
			   step(-0.01,worldPos.y) * lerp(color, skyFogColor, _FogSkyAlpha * saturate((_FogSkyHaze/worldPos.y)*(1-skyNoise*_FogSkyNoise)));
	}
