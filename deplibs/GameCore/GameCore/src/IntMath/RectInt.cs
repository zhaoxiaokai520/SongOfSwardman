...
public struct RectInt
{
    ...
    public Rect rec
    {
        get
        {
            return new Rect((float)m_XMin * PrecisionFactor, (float)m_YMin * PrecisionFactor, (float)m_Width * PrecisionFactor,(float)m_Height * PrecisionFactor);
        }
    }
	public VecInt2 position
	{
		get
		{
			return new VecInt2(this.m_XMin, this.m_YMin);
		}
		set
		{
			this.m_XMin = value.x;
	...
	public VecInt2 center
	{
		get
		{
			return new VecInt2(this.x + (this.m_Width >> 1), this.y + (this.m_Height >> 1));
		}
		set
		{
	...
	public VecInt2 min
	{
		get
		{
			return new VecInt2(this.xMin, this.yMin);
		}
		set
		{
			this.xMin = value.x;
	...
	public VecInt2 max
	{
		get
		{
			return new VecInt2(this.xMax, this.yMax);
		}
		set
		{
			this.xMax = value.x;
	...
	public VecInt2 size
	{
		get
		{
			return new VecInt2(this.m_Width, this.m_Height);
		}
		set
		{
    ...
    public static RectInt MinMaxRect(int left, int bottom, int right, int top)
	{
	...
	public bool Contains(VecInt2 point)
	{
		return point.x >= this.xMin && point.x < this.xMax && point.y >= this.yMin && point.y < this.yMax;
	}

	public bool Contains(VecInt3 point)
	{
		return point.x >= this.xMin && point.x < this.xMax && point.y >= this.yMin && point.y < this.yMax;
	}

    public bool Contains2D(VecInt3 point)
    {
        return point.x >= this.xMin && point.x < this.xMax && point.z >= this.yMin && point.z < this.yMax;
    }

	public bool Contains(VecInt3 point, bool allowInverse)
	{
		if (!allowInverse)
		{
    ...
    {
        return new RectInt((int)Math.Round((double)(rect.x * 1000f)), (int)Math.Round((double)(rect.y * 1000f)), (int)Math.Round((double)(rect.width * 1000f)), (int)Math.Round((double)(rect.height * 1000f)));
    }

    public static explicit operator Rect(RectInt rect)
    {
        return new Rect((float)(rect.x * 0.001f), (float)(rect.y * 0.001f), (float)(rect.width * 0.001f), (float)(rect.height * 0.001f));
    }
